-- Fix the get_class_deletion_counts function to use correct column names
...
BEGIN
  -- Count students (via student_enrollments) - using user_id
  ...
  -- Count bids (via opportunities) - FIXED: ensure we use user_id, not student_id
  SELECT COUNT(*)::integer INTO v_bids
  FROM bids b
  INNER JOIN opportunities o ON b.opportunity_id = o.id
  WHERE o.class_id = p_class_id;
  
  -- Count token history (via opportunities) - FIXED: ensure we use user_id, not student_id
