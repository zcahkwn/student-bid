...
BEGIN
  ...
  v_opportunity_capacity := COALESCE(v_opportunity_record.capacity, v_opportunity_record.capacity_default, 7);
...
BEGIN
  ...
  v_result := jsonb_build_object(
    ...
    'opportunity_details', jsonb_build_object(
      ...
      'capacity', COALESCE(v_opportunity.capacity, v_opportunity.capacity_default, 7),
    ...
    'bid_statistics', jsonb_build_object(
      ...
      'capacity_exceeded', COALESCE(v_bid_stats.total_bids, 0) > COALESCE(v_opportunity.capacity, v_opportunity.capacity_default, 7)
    ),
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'opportunity_details', jsonb_build_object(
      ...
      'opens_at', v_opportunity.opens_at,
      'closes_at', v_opportunity.closes_at,
      'capacity', COALESCE(v_opportunity.capacity, v_student.capacity_default, 7)
    ),
    'checked_at', NOW()
  );
END;
