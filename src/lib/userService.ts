...
const updateStudentEnrollment = async (
  userId: string,
  classId: string,
  updates: Partial<Omit<StudentEnrollment, 'userId' | 'classId'>>
): Promise<void> => {
  try {
    const updateData: any = {}
    
    if (updates.tokensRemaining !== undefined) updateData.tokens_remaining = updates.tokensRemaining
    if (updates.tokenStatus) updateData.token_status = updates.tokenStatus
    if (updates.biddingResult) updateData.bidding_result = updates.biddingResult
...
const removeUserFromClass = async (userId: string, classId: string): Promise<void> => {
  try {
    const { error } = await supabase
      .from('student_enrollments')
      .delete()
...
// Convert User + StudentEnrollment to Student interface for backward compatibility
const convertToStudent = (user: User, enrollment: StudentEnrollment): Student => {
  return {
    id: user.id,
    name: user.name,
    email: user.email,
